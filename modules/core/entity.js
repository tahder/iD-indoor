...
Entity.parseLevelsTags = function(tags) {
	...
	//Tag level
		...
		levels = Entity.parseLevelString(tags.level);
	}
	//Tag repeat_on
	else if(tags.repeat_on != undefined) {
		levels = Entity.parseLevelString(tags.repeat_on);
	}
	//Tag min_level and max_level
	else if(tags.min_level != undefined && tags.max_level != undefined) {
		levels = Entity.parseLevelString(tags.min_level+"-"+tags.max_level);
	}
	//Tag buildingpart:verticalpassage:floorrange
	else if(tags["buildingpart:verticalpassage:floorrange"] != undefined) {
		levels = Entity.parseLevelString(tags["buildingpart:verticalpassage:floorrange"]);
	}
...
Entity.parseLevelString = function(str) {
	var result = null;
	...
	//Level intervals
		...
		if(Entity.lvlRgx3.test(str)) {
			regexResult = Entity.lvlRgx3.exec(str);
			min = parseInt(regexResult[1]);
			max = parseInt(regexResult[2]);
		}
		else if(Entity.lvlRgx4.test(str)) {
			regexResult = Entity.lvlRgx4.exec(str);
			min = parseInt(regexResult[1]);
			max = parseInt(regexResult[2]);
		}
...
Entity.prototype = {
    ...
    initialize: function(sources) {
		...
		this.levels = Entity.parseLevelsTags(this.tags);
